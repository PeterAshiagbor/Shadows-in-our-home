  ...
  if (req.method === 'POST') {
    ...
    if (body.photo?.base64) {
      ...
      if (delegate.photo_path) {
        // Replacing a photo: the old object must go, or it stays fetchable in
        // the public bucket after photo_path is overwritten. Unlike the admin
        // paths this does not fail the request — the delegate's save should not
        // block on a storage hiccup — but the orphaned path is logged so it can
        ...
        if (removeError) {
          console.error(
            'ORPHANED PHOTO — replaced object left in public bucket:',
            delegate.photo_path,
            removeError.message
          )
        }
